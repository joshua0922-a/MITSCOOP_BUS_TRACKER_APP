...
if(!bus_number){
return res.status(400).json({error:"Bus number required"});
}

db.run(
`INSERT INTO buses (bus_number,plate_number,route,driver,conductor,status)
VALUES (?,?,?,?,?,?)`,
...
db.run(
`UPDATE buses
SET plate_number=?,route=?,driver=?,conductor=?,status=?
WHERE id=?`,
...
if(!bus_number || !action){
return res.status(400).json({
error:"bus_number and action required"
});
}
...
db.run(
`INSERT INTO logs (bus_number,action,time,date,remarks)
VALUES (?,?,?,?,?)`,
...
db.run(
`UPDATE logs SET remarks=? WHERE id=?`,
...
app.listen(PORT, ()=>{
console.log(`MITSCOOP Bus Tracker running on http://localhost:${PORT}`);
});